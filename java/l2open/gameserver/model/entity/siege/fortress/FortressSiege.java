...
public class FortressSiege extends Siege
{
	...
	{
		if (((Fortress) getSiegeUnit()).getId() == 104){
			_siegeDate = Calendar.getInstance();
			if (Calendar.getInstance().getTimeInMillis() - getSiegeUnit().getLastSiegeDate() * 1000L > 3600000) {
				_siegeDate.set(_siegeDate.get(Calendar.YEAR), _siegeDate.get(Calendar.MONTH), _siegeDate.get(Calendar.DAY_OF_MONTH), 21, 0, 0);
			}else {
				_siegeDate.set(_siegeDate.get(Calendar.YEAR), _siegeDate.get(Calendar.MONTH), _siegeDate.get(Calendar.DAY_OF_MONTH) + 1, 21, 0, 0);
			}
			_database.saveSiegeDate();
		}
